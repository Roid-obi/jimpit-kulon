...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-primary flex flex-col items-center justify-center overflow-hidden relative">
      ...
      <div className={`absolute z-0 flex flex-col items-center gap-4 transition-opacity duration-700 ${expand ? 'opacity-0 delay-300' : 'opacity-100'}`}>
         <Image
      ...
      <main 
        ...
        style={{
          clipPath: expand ? 'circle(150% at 50% 50%)' : 'circle(0% at 50% 50%)'
        }}
      >
         <Image
            src="/jimpit-kulon-logo.svg"
        ...
        <p className="text-lg max-w-md text-foreground/80">
          Sistem pengelolaan jimpitan warga yang transparan, aman, dan mudah digunakan.
        </p>
        
        <div className="flex gap-4 mt-8">
          <button className="px-8 py-3 rounded-full bg-primary text-foreground font-semibold hover:opacity-90 transition-opacity shadow-sm cursor-pointer">
            Mulai Sekarang
          </button>
          <button className="px-8 py-3 rounded-full bg-secondary text-foreground font-semibold hover:opacity-90 transition-opacity shadow-sm cursor-pointer">
            Pelajari
          </button>
        </div>
      </main>
